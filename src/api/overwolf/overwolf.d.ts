...
export namespace ODK {
  ...
  namespace GameEvents {
    ...
    type InfoCallback<F extends AvailableFeaturesMap, T extends GameEventsInfoDB<F>> = (
      arg: InfoCBArg<F, T>
    ) => any;
    type InfoCBArg<
      F extends AvailableFeaturesMap,
      T extends GameEventsInfoDB<F>
    > = ODKCallbackArg & { /** shown when status is "error" */ reason?: string } & { res: T };

    type NewEventsCallback<E extends EventData<TEvents>> = (arg: EventUpdate<E>) => void;
    type InfoUpdateCallback<F extends TFeatures, I extends InfoUpdateData> = (
      arg: InfoUpdate<F, I>
    ) => void;
    ...
    namespace LOL {
      ...
      type GameEventsInfoDBLOL = ODK.GameEvents.GameEventsInfoDB<AvailableFeaturesMapLOL> &
        InfoUpdateDataLOL;
...
//////
/** An Overwolf event. */
export interface OverwolfEventArgs {
  // TODO: remove this in favor of ODKCallbackArg
...
export interface OverwolfWindows {
  ...
  obtainDeclaredWindow(
    windowId: ODKWindowId,
    callback: (arg: ODKCallbackArg & { window: ODKWindow }) => void
  ): void;
  ...
  close(
    windowId: ODKWindowId,
    callback?: (arg: ODKCallbackArg & { window_id: string }) => void
  ): void;
   ...
   */
  getWindowsStates(
    callback: (arg: ODKCallbackArg & { result: { [windowName: string]: ODKWindowStates } }) => void
  ): void;
  ...
  setTopmost(
    ...
    callback: (arg: ODKCallbackArg) => void
  ): void;
  ...
  isWindowVisibleToUser(
    callback: (arg: ODKCallbackArg & { visible: 'hidden' | 'fully' | 'partial' }) => void
  ): void;
...
/// overwolf.media
///////
export interface OverwolfMedia {
  // TODO: update, lots of added methods
  /**
...
export interface OverwolfWeb {
  ...
  createServer(
    port: number,
    cb: (arg: ODKCallbackArg & { server: OverwolfWebServer }) => void
  ): void;
...
export interface OverwolfGames {
  ...
  events: {
     ...
     * @since 0.93.1 */
    setRequiredFeatures(
      features: ODK.GameEvents.TFeatures[],
      callback?: (
        arg: ODKCallbackArg & {
     ...
     */
    getInfo<
      F extends ODK.GameEvents.AvailableFeaturesMap,
      T extends ODK.GameEvents.GameEventsInfoDB<F>
    >(
      callback: ODK.GameEvents.InfoCallback<F, T>
     ...
     */
    onInfoUpdates2: {
      addListener<F extends ODK.GameEvents.TFeatures, I extends ODK.GameEvents.InfoUpdateData>(
        callback: ODK.GameEvents.InfoUpdateCallback<F, I>
      ): void; // TODO: check if generics are needed here
      removeListener<F extends ODK.GameEvents.TFeatures, I extends ODK.GameEvents.InfoUpdateData>(
        callback: ODK.GameEvents.InfoUpdateCallback<F, I>
      ): void; // TODO: check if generics are needed here
    };
    /**
    ...
    onNewEvents: {
      addListener(
        callback: ODK.GameEvents.NewEventsCallback<ODK.GameEvents.EventData<ODK.GameEvents.TEvents>>
      ): void;
      removeListener(
        callback: ODK.GameEvents.NewEventsCallback<ODK.GameEvents.EventData<ODK.GameEvents.TEvents>>
      ): void;
  ...
  inputTracking: {
     ...
     * @since 0.78
     */
    onMouseUp: ODKListenable<{
      button: TODKMouseButton;
      onGame: boolean;
      x: number;
      y: number;
    }> /** TODO:
...
/** @updated 2016/12/20 Client v0.101.15 */
export interface ODKStaticGameInfo {
  // http://developers.overwolf.com/documentation/sdk/overwolf/games/#getgameinfo
  ...
  GameInfo: {
    // taken from call for League of Legends (GameClassId 5426)
...
/** @updated 2016/12/20 Client v0.101.15 */
export interface ODKRunningGameInfo {
  // http://developers.overwolf.com/documentation/sdk/overwolf/games/gameinfo/
...
export interface OverwolfExtensions {
  ...
  getService<TServiceType>(
    id: string,
    callback: (args: LocateServiceEvent<TServiceType>) => void
  ): void; // TODO: check if (T) generics are needed here
   ...
   * @since 0.92.300
   */
  onAppLaunchTriggered: ODKListenable<{
    origin: 'dock' | 'storeapi' | 'odk' | string;
    parameter: any;
  }>;
}
...
export interface ODKManifest {
  // TODO - update with http://developers.overwolf.com/documentation/sdk/overwolf/manifest-json/ (complete rework necc.)
  // "manifest_version": 1,
  // "type": "WebApp",
...
/// overwolf.utils
//////
export interface OverwolfUtils {
  // http://developers.overwolf.com/documentation/sdk/overwolf/utils
...
export interface OverwolfIO {
  // TODO http://developers.overwolf.com/documentation/sdk/overwolf/io/
  fileExists: (arg: any, callback: any) => any; // TODO
  writeFileContents: (
    filePath: any,
    content: any,
    encoding: any,
    triggerUacIfRequired: any,
    callback: any
  ) => any; // TODO
  /** @undocumented */
...
export interface SimpleIOPlugin extends HTMLElement {
  // TODO: typings for plugins - propably best in own projects
  ...
  getTextFile: (
    filePath: string,
    isUCS2: boolean,
    callback: (status: boolean, data: string) => void
  ) => void;
  ...
  getBinaryFile: (
    filePath: string,
    limit: number,
    callback: (status: boolean, data: ArrayBuffer) => void
  ) => void;
   ...
   * @param callback
   */
  writeLocalAppDataFile: (
    filePathWithinLocalAppData: string,
    content: string,
   ...
   */
  onFileListenerChanged: ODKGenericListenable<
    (listeningId: string, status: boolean, data: string) => void
  >;
  /** TODO: update documentation when https://github.com/overwolf/overwolf-plugins/issues/3 got solved
  ...
  getLatestFileInDirectory: (
    path: string,
    callback: (status: boolean, filename: string) => void
  ) => void;
  /**
  ...
  listDirectory: (
    path: string,
    callback: (status: boolean, listing: ODKFileToken[]) => void
  ) => void;
}
